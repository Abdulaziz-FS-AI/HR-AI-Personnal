...
export async function POST() {
  try {
    console.log('Creating evaluation schema...')
    ...
    await pool.request().query(`
      ...
      CREATE TABLE evaluation_sessions (
        ...
        status NVARCHAR(50) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'completed', 'failed')),
    ...
    await pool.request().query(`
      ...
      CREATE TABLE evaluation_files (
        ...
        status NVARCHAR(50) NOT NULL DEFAULT 'uploaded' CHECK (status IN ('uploaded', 'processing', 'completed', 'failed')),
    ...
    return NextResponse.json({
      ...
      indexes: [
        'IX_evaluation_sessions_user_id',
        'IX_evaluation_files_evaluation_id',
        'IX_evaluation_results_evaluation_id'
      ],
